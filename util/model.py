...
class Unet:
    ...
    def create_model(size, l2_reg):
        ...
        teacher = tf.placeholder(
            # tf.float32, [None, size[0], size[1], len(ld.DataSet.CATEGORY)])
        ...
        concated1 = tf.concat([UNet.conv_transpose(
        ...
        concated2 = tf.concat([UNet.conv_transpose(
        ...
        concated4 = tf.concat([UNet.conv_transpose(
    ...
    def conv(inputs, filters, kernel_size=[3, 3], activation=tf.nn.relu, l2_reg_scale=None, batchnorm_istraining=None):
        if l2_reg_scale is None:
            ...
            regularizer = tf.contrib.layers.l2_regualrizer(scale=l2_reg_scale)
    ...
    def pool(inputs):
        pooled = tf.layers.max_pooling2d(
            inputs=inputs, pool_size=[2, 2], stride=2)
        return pooled
