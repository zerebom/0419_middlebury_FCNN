...
class Reporter:
    ...
    def save_image_from_ndarray(self, train_set, test_set, epoch, index_void=None):
        assert len(train_set) == len(test_set) == 3
        train_image = Reporter.get_imageset(
            train_set[0], train_set[1], train_set[2], index_void)
        test_image = Reporter.get_imageset(
            test_set[0], test_set[1], test_set[2], index_void)
    ...
    @staticmethod
    def concat_images(im1, im2, mode):
        if mode == "P":
            pass
    ...
    @staticmethod
    def cast_to_pil(ndarray, index_void=None):
        #
    ...
    @staticmethod
    def get_imageset(image_in_np, image_out_np, image_tc_np, index_void=None):
        assert image_in_np.shape[:2] == image_out_np.shape[:2] == image_tc_np.shape[:2]
        image_out, image_tc = Reporter.cast_to_pil(image_out_np, index_void),\
            Reporter.cast_to_pil(image_tc_np, index_void)
        image_concated = Reporter.concat_images(image_out, image_tc, "RGB").convert("RGB")
        # npから画像を作成する
        image_in_pil = Image.fromarray(np.uint8(image_in_np * 255), mode="RGB")
        image_result = Reporter.concat_images(image_in_pil, image_concated,"RGB")
        return image_result
...
class MatPlotManager:
    ...
    def add_figure(self, title, xylabels, labels, filename=None):
        ...
        self._figures[title] = MatPlot(
            title, xylabels, labels, self._root_dir, filename=filename)
